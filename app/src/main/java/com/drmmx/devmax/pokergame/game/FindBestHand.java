...
class FindBestHand {
    private static ArrayList<Integer> whereExists(int start, int end, Integer[] findIn) {
    ...
    private static ArrayList<Integer> isStraight(ArrayList<Integer> positions) {
        ArrayList<Integer> output = new ArrayList();
        if (positions.size() == 5) {
    ...
    private static ArrayList<Integer> flushHighCards(CardSet limitSuit, String s) {
        ArrayList<Integer> output = new ArrayList();
        ...
        for (int i = 0; i < cardSize; i++) { //count amount of cards with each value
            switch (cards.getFirst().getValue()) {
                case "2":
                    countByValue[1]++;
    ...
    static ArrayList<Integer> findBestHand(CardSet cards) {
